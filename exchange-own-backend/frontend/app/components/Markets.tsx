...
export const Markets = () => {
  const [tickers, setTickers] = useState<Ticker[] | undefined>(undefined); // Allow undefined as an initial state

  useEffect(() => {
    getTickers().then((m) => {
      console.log("Tickers:", m);
      setTickers(Array.isArray(m) ? m : []); // Ensure tickers is always an array
    });
  }, []);
